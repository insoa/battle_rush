...
namespace Pathfinding {
	...
	public class GridGraphEditor : GraphEditor {
		...
		bool IsHexagonal(GridGraph graph) {
			return Mathf.Approximately(graph.isometricAngle, standardIsometric) && graph.neighbours == NumNeighbours.Six &&
		...
		void DrawFirstSection(GridGraph graph) {
			...
			if (graph.inspectorGridMode == InspectorGridMode.Hexagonal) {
				...
				graph.inspectorHexagonSizeMode = (InspectorGridHexagonNodeSize) EditorGUILayout.EnumPopup(
		...
		protected virtual void DrawNeighbours(GridGraph graph) {
			...
			graph.neighbours = (NumNeighbours) EditorGUILayout.Popup(
		...
		void DrawPenaltyModifications(GridGraph graph) {
			...
			if (showExtra) {
				...
				graph.penaltyAngle = ToggleGroup(
					new GUIContent("Angle Penalty", "Adds a penalty based on the slope of the node"),
					graph.penaltyAngle
				);
		...
		protected virtual void DrawJPS(GridGraph graph) {
			graph.useJumpPointSearch = EditorGUILayout.Toggle(
				new GUIContent(
					"Use Jump Point Search",
					"Jump Point Search can significantly speed up pathfinding. But only works on uniformly weighted graphs"
				),
				graph.useJumpPointSearch
			);
			if (graph.useJumpPointSearch) {
				EditorGUILayout.HelpBox(
					"Jump Point Search assumes that there are no penalties applied to the graph. Tag penalties cannot be used either.",
					MessageType.Warning
				);

#if !ASTAR_JPS
				EditorGUILayout.HelpBox(
					"JPS needs to be enabled using a compiler directive before it can be used.\n" +
					"Enabling this will add ASTAR_JPS to the Scriping Define Symbols field in the Unity Player Settings",
					MessageType.Warning
				);
		...
		protected virtual void DrawCollisionEditor(GraphCollision collision) {
			...
			if (collision.collisionCheck) {
				string[] colliderOptions = {"Sphere", "Capsule", "Ray"};
				int[] colliderValues = {0, 1, 2};
		...
		protected virtual void DrawTextureData(GridGraph.TextureData data, GridGraph graph) {
			...
			for (int i = 0; i < 3; i++) {
				...
				data.channels[i] = (GridGraph.TextureData.ChannelUse) EditorGUILayout.Popup(
					channelName,
					(int) data.channels[i],
					ChannelUseNames
				);
					...
					switch (data.channels[i]) {
						...
						case GridGraph.TextureData.ChannelUse.Position:
							...
							if (graph.collision.heightCheck) {
								EditorGUILayout.HelpBox(
									"Getting position both from raycast and from texture. You should disable one of them",
									MessageType.Error
								);
							}
						...
						case GridGraph.TextureData.ChannelUse.WalkablePenalty:
							help = "If channel '" + channelName +
							       "' is 0, the node is made unwalkable. Otherwise the node is applied penalty multiplied with factor";
							break;
					}
		...
		public static GridPivot PivotPointSelector(GridPivot pivot) {
			...
			gridPivotSelectBackground =
				gridPivotSelectBackground ?? AstarPathEditor.astarSkin.FindStyle("GridPivotSelectBackground");
		...
		static readonly Vector3[] handlePoints = new[]
			{new Vector3(0.0f, 0, 0.5f), new Vector3(1.0f, 0, 0.5f), new Vector3(0.5f, 0, 0.0f), new Vector3(0.5f, 0, 1.0f)};
			...
			if (Tools.current == Tool.Scale) {
				...
				for (int i = 0; i < handlePoints.Length; i++) {
					...
					Vector3 p = savedTransform.InverseTransform(
						Handles.Slider(ps, ps - center, HandleScale * HandleUtility.GetHandleSize(ps), cap, 0)
					);
				...
				center = Handles.PositionHandle(
					graph.center,
					Tools.pivotRotation == PivotRotation.Global ? Quaternion.identity : Quaternion.Euler(graph.rotation)
				);
